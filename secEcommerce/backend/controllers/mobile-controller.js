...
export const addMobile = async function addMobile(req,res) {
    ...
    try{
        ...
        const {name,brand,rom,ram,price,color,qty,image_arr} = req.body

        if (!name || !brand || !rom || !ram|| !price || !color || !image_arr || !qty) {
            return res.status(404).send({ error: "please fill all fields" })
        }


        const data = await mobileSchema.create({ name, brand, rom, ram, price, color, qty,image_arr });
